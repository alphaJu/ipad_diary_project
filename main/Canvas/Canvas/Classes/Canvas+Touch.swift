...
public extension Canvas {
    ...
    public override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        case .eyedropper:
            return
        case .pen:
            lastLastPoint = midpoint(a: lastLastPoint, b: lastPoint)
        ...
        case .eraser:
            ...
            for var i in 0..<currLayer.drawingArray.count {
                ...
                case 0:
                    ...
                    /*let erasedInstructions = instructions.map { item -> Bool in
                    ...
                    let erasedInstructions = instructions.map {(item : ([CGPoint], CGPathElementType)) -> ([CGPoint], CGPathElementType) in
                        let contains = item.0.contains { (cgPoint: CGPoint) -> Bool in
                            return cgPoint.inRange(of: erasePoint, by: 5)
                        }
                        
                        if (contains){
                             return (item.0, .moveToPoint)
                        }
                        return item
                    }
                    
                    /*let erasedInstructions = instructions.filter { (cgPoints : [CGPoint], type: CGPathElementType) -> Bool in
                        if (cgPoints.contains(erasePoint)){
                            print("contains")
                            return true
                        }
                   ...
                   /* let split_index = instructions.index { (cgPoints : [CGPoint], type: CGPathElementType) -> Bool in
                        if (cgPoints.contains(erasePoint)){
                            return true
                        }
                        return false
                    ...
                    var new_node1 = node.copy()
                    var new_node2 = node.copy()
                    ...
                    */
                    ...
                    if let nPath = buildPath(from: erasedInstructions.map { $0.1 }, bPoints: erasedInstructions.map { $0.0 }, type: .pen){
